...
import in.dudeapp.collegeservice.service.impl.CollegeServiceImpl;
...
class CollegeControllerTest {
    ...
    void setUp() {
        ...
        //pageDTO.setTotalResults(2);
        //collegeResponse.setData(List.of(collegeDTO,collegeDTO2));
        //collegeResponse.setPage(pageDTO);
    ...
    void getCollegesByName() throws Exception {
        String requestString = "/getCollegesByName?name=c";
    ...
    void updateCollege() throws Exception {
        String contentString = writer.writeValueAsString(collegeRequest1);
        String requestString = "/5";
        willDoNothing().given(collegeService).updateCollege(any(CollegeRequest.class),anyInt());
    ...
    void getCollegeById() throws Exception {
        ...
        when(collegeService.getCollegeById(anyInt())).thenReturn(collegeDTO);
        mockMvc.perform(get("/2").contentType(MediaType.APPLICATION_JSON))
                .andExpect(status().isOk())
                .andExpect(content().string(responseString));
    }
    ...
    void deleteCollege() throws Exception {
        willDoNothing().given(collegeService).deleteCollegeById(anyInt());
        mockMvc.perform(delete("/3").contentType(MediaType.APPLICATION_JSON))
                .andExpect(status().isOk())
                .andExpect(content().string("College Deleted Successfully!"));
    }

    private CollegeDTO generateDTO(int id, String name,
    ...
    private CollegeRequest generateRequest(int id, String name,
                                   String street, String city,
                                   String district, String state,
                                   int pincode) {
